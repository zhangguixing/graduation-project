...
<head>
	...
	$(function() {
		...
		$.ajax({
				type: "post",
				url: remoteUrl+"student/getStudentDetail&t="+new Date().getTime(),
	    	...
	    	buttons: [
	  	    		{
	  					...
	  					handler:function(){
	  						...
	  						if(!validate){
	  							...
	  							$.ajax({
	  								type: "post",
	  								url: remoteUrl+"system/editPasswod&t="+new Date().getTime(),
	  								data: $("#editPassword").serialize(),
	  								success: function(res){
	  									if(res.status == 0){
	  										$.messager.alert("消息提醒","修改成功，将重新登录","info")
	  										setTimeout(function(){
	  											top.location.href = remoteUrl+"system/loginOut";
	  										}, 1000);
	  									}
	  								}
	  							});
	    	...
	    	toolbar: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							$.ajax({
								type: "post",
								url: remoteUrl+"student/editStudent&t="+new Date().getTime(),
		...
		$("#uploadBtn").click(function(){
			...
			setTimeout(function(){
				...
				$("#user_photo").attr("src", remoteUrl+"photo/getPhoto&t="+new Date().getTime());
			}, 1500)
		});
...
<body>
	...
	<div id="editDialog" style="padding: 20px">
		<div style="width: 300px; height: 400px;float: right; margin: 20px 120px 0 0;">
	    	<img id="user_photo" alt="照片" style="margin-bottom: 30px;display: block;max-width: 250px; max-height: 300px;" title="照片" src="/photo/getPhoto" />
	    	<form id="uploadForm" method="post" enctype="multipart/form-data" action="photo/setPhoto" target="photo_target">
