...
<head>
	...
	$(function() {	
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        fit: true,//自动大小 
	        method: "post",
	        url:remoteUrl+"exam/teacherExamList&t="+new Date().getTime(),
	        idField:'id', 
	    ...
	    $("#add").click(function(){
	    	...
	    	$.ajax({
				type: "post",
				url: remoteUrl+"teacher/getTeacher&t="+new Date().getTime(),
				dataType: "json",
            	...
            	if(exam.type == 2){
            		$("#regClazzList").combobox("readonly", true);
               		...
               		setTimeout(function(){
    	            	...
    	            	$.ajax({
    	            		type: "post",
    						url: remoteUrl+"score/columnList",
    						data: data,
    						dataType: "json",
    						async: false,
    						success: function(result){
                		...
                		//加载数据
    			    	setTimeout(function(){
    				    	$("#regEscoreList").datagrid("options").url = remoteUrl+"score/scoreList&t="+new Date().getTime();
	            	...
	            	$("#regClazzList").combobox("options").url = remoteUrl+"teacher/getExamClazz";
	    ...
	    $("#escore").click(function(){
        	...
        	if(exam == null){
            	...
            	setTimeout(function(){
			    	$("#escoreList").datagrid("options").url = remoteUrl+"score/scoreList&t="+new Date().getTime();
	    	...
	    	buttons: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							$.ajax({
								type: "post",
								url: remoteUrl+"exam/addExam&t="+new Date().getTime(),
								data: data,
	    ...
	    $("#regEscoreDialog").dialog({
	  	...
	  	$("#redo").click(function(){
        	...
        	var url = remoteUrl+"score/exportScore&id="+exam.id+"&gradeid="+exam.gradeid+"&clazzid="+clazzid+"&courseid="+exam.courseid+"&type="+exam.type;
	  	...
	  	$("#escoreClazzList").combobox({
	  		...
	  		editable: false, //不可编辑
	  		method: "post",
	  		url: remoteUrl+"clazz/clazzList&t="+new Date().getTime(),
	  	...
	  	$("#regiser").click(function(){
	  		...
	  		$.ajax({
				type: "post",
				url: remoteUrl+"score/setScore&t="+new Date().getTime(),
				data: {score: score},
				success: function(res){
					if(res.status == 0){
	  	...
	  	$("#regClazzList").combobox({
	  		...
	  		onChange: function(newValue, oldValue){
	  			...
	  			$("#regCourseList").combobox("options").url = remoteUrl+"teacher/getExamCourse&t="+new Date().getTime();
