...
<head>
	...
	$(function() {	
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        fit: true,//自动大小 
	        method: "post",
	        url:remoteUrl+"teacher/teacherList&t="+new Date().getTime(),
	        idField:'id', 
        	...
        	if(selectLength == 0){
            	...
            	$.messager.confirm("消息提醒", "将删除与教师相关的所有数据，确认继续？", function(r){
            		if(r){
            			$.ajax({
							type: "post",
							url: remoteUrl+"teacher/deleteTeacher",
				...
				},
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							$.ajax({
								type: "post",
								url: remoteUrl+"teacher/addTeacher",
								data: data,
	  ...
	  //下拉框通用属性
	  	...
	  	$("#add_courseList").combobox({
	  		url: remoteUrl+"course/courseList&t="+new Date().getTime(),
	  		onLoadSuccess: function(){
		  		//默认选择第一条数据
				var data = $(this).combobox("getData");;
				$(this).combobox("setValue", data[0].id);
	  	...
	  	$("#editDialog").dialog({
				...
				},
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							$.ajax({
								type: "post",
								url: remoteUrl+"teacher/editTeacher",
								data: data,
								success: function(msg){
									if(msg == "success"){
			...
			onBeforeOpen: function(){
				...
				$("#edit_photo").attr("src", remoteUrl+"photo/getPhoto&type=3&number="+selectRow.number);
