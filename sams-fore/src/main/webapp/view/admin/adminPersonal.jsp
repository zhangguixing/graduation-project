	...
	$(function() {	
		...
		$("#systemTable a").each(function(){
			$(this).click(function(){
				...
				$.ajax({
					type: "post",
					url: remoteUrl+"system/editSystemInfo&t="+new Date().getTime(),
					data: data,
					success: function(res){
						if(res.status == 0){
    	...
    	<form id="editPassword">
	    	<table cellpadding="8" >
	    		<tr>
	    			<td>原密码:</td>
	    			<td>
	    				<input id="old_password" style ="width: 200px; height: 30px;" class="easyui-textbox" type="password" validType="oldPassword[${sessionUser.password}]"  data-options="required:true, missingMessage:'请输入原密码'" />
	    			</td>
	    		</tr>
	    		<tr>
	    			<td>新密码:</td>
	    			<td>
	    				<input  type="hidden" name="account" value="${sessionUser.account }" />
